...
public class SimulationStructureOperators {
     ...
     public static void ApplyTreatment(int treatNb,SimulationStructure simulation){
        ...
        for (int i=0;i<simulation.theta[simulation.currentSolution].tab.length;i++){
            ind=createFunction(simulation.theta[simulation.currentSolution-1].tab[i].min,simulation.theta[simulation.currentSolution-1].tab[i].max,simulation.theta[simulation.currentSolution-1].tab[i].step);
            simulation.theta[simulation.currentSolution].tab[i]=Operators.MultiplyFunctions(simulation.theta[simulation.currentSolution-1].tab[i],ind);
            for (int j=simulation.theta[simulation.currentSolution].tab[i].minIndex;j<simulation.theta[simulation.currentSolution].tab[i].maxIndex;j++){
                simulation.theta[simulation.currentSolution].tab[i].values[j]*=survivalProbability.op(simulation.treat.doses[treatNb],i);
            }
        }
